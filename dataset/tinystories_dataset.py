...
class TinyStoriesDataset(DiskDataset):
    ...
    def get_splits(tokenizer, max_seq_len, batch_size):
        return {
            "train": TinyStoriesDataset("train", tokenizer, max_seq_len, do_shuffle=True, batch_size=batch_size),
            "val": TinyStoriesDataset("val", tokenizer, max_seq_len, batch_size=batch_size),
            "test": TinyStoriesDataset("test", tokenizer, max_seq_len, batch_size=batch_size)
        }